...
from src.tests.fit_analysis_helpers import make_job
from src.tools.implementations.fit_analysis import verdict
from src.tools.implementations.fit_analysis.render import render_fit_analysis
...
def _sample() -> FitAnalysisOutput:
    return FitAnalysisOutput(
        job_id="J1",
        aligned_skills=[
            EvidenceClaim(claim="Python: aligned", evidence_ids=["e1"], notes=verdict.tag(verdict.MATCH))
        ],
        ...
        genuine_gaps=[EvidenceClaim(claim="Rust: gap", notes=verdict.tag(verdict.MISMATCH))],
    )
...
def test_render_contains_all_sections() -> None:
    ...
    for section in ("Relevant Experience", "Seniority", "Education", "Core Skills", "Projects"):
        assert f"## {section}" in text
    assert "✅ Python: aligned" in text
    assert "❌ Rust" in text  # genuine gap rendered skill-only


def test_missing_group_marker_is_configurable() -> None:
    output = _sample()
    assert "❌ deep learning" in render_fit_analysis(output, make_job(), missing_marker="❌")
    assert "➕ deep learning" in render_fit_analysis(output, make_job(), missing_marker="➕")
...
def test_missing_skill_shows_human_readable_source_inline() -> None:
    ...
    assert "_[evidence: portfolio-P04]_" in text  # IDs kept, after the readable citation


def test_partial_renders_as_cross_not_positive() -> None:
    # A seniority shortfall (verdict=partial) must render ❌ in markdown, never ✅.
    output = FitAnalysisOutput(
        job_id="J1",
        seniority=[
            EvidenceClaim(claim="Seniority: 4y vs 5+", notes=verdict.tag(verdict.PARTIAL))
        ],
    )
    text = render_fit_analysis(output, make_job())
    assert "❌ Seniority: 4y vs 5+" in text
    assert "✅" not in text  # only ✅ and ❌ may appear; partial is not ✅
    assert "⚠️" not in text  # no warning glyph in the rendered report


def test_render_states_no_swap_branch_explicitly() -> None:
    text = render_fit_analysis(FitAnalysisOutput(job_id="J1", project_swap=None), make_job())
...
def test_render_shows_recommended_swap() -> None:
    output = FitAnalysisOutput(
        ...
        project_swap=ProjectSwap(
            remove_project="Old", add_project="New", rationale="stronger", evidence_ids=["portfolio-P2"]
        ),
    )
    text = render_fit_analysis(output, make_job())
