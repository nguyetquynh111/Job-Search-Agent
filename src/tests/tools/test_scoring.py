...
from src.tools.scoring import score_jobs
...
def _run(jobs: list[Job], profile: CandidateProfile, **evidence: object) -> object:
    return score_jobs(ScoreJobsInput(jobs=jobs, candidate_profile=profile, **evidence))
...
def test_documented_weighted_formula_has_an_exact_result() -> None:
    ...
    result = _run([job], profile).ranked_jobs[0]
