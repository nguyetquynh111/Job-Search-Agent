...
from src.tests.fit_analysis_helpers import make_evidence, make_input, make_job, make_profile
from src.tools.implementations.fit_analysis import llm_client, reasoning
...
_VALID_JSON = json.dumps(
    {
        ...
        "education": [],
        "aligned_skills": [
            {"claim": "Python: aligned", "evidence_ids": ["resume-skills-001"], "confidence": 0.8, "notes": None}
        ],
...
def test_stub_llm_produces_llm_path() -> None:
    ...
    output, path, _ = reasoning.analyze(inp, prepass, index, complete_fn=lambda s, u: _VALID_JSON)
    assert path == "llm"
