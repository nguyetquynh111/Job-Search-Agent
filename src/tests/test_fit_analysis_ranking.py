...
from src.tools.implementations.fit_analysis.prepass import _expand_canonicals
from src.tools.implementations.fit_analysis.sanitize import sanitize_text
from src.tools.implementations.fit_analysis.swap import _matches, rank_projects
...
pytestmark = pytest.mark.skipif(
    not (_JOBS.exists() and _PORTFOLIO.exists()), reason="repository data files not present"
)
...
def test_j030_prefers_the_recommendation_project_over_medical_imaging() -> None:
    ...
    # "Deep Learning" is filler-only overlap and must no longer count as alignment.
    assert by_id["P04"].domain_matches == []


def test_partial_phrase_overlap_needs_a_distinctive_word() -> None:
    # One distinctive word carries a phrase...
    assert _matches(["Recommendation and Ranking"], "we build recommendation systems") == [
        "Recommendation and Ranking"
    ]
    # ...but filler words alone never do.
    ...
    # Plurals stem, so "systems" and "system" agree.
