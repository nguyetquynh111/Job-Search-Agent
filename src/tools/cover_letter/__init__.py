"""Production implementation of the ``generate_cover_letter`` model-visible tool.
...
Design notes
------------
* No fact used in the letter is invented. Candidate claims cite
  ``value.candidate_evidence`` (resume, master skills, portfolio, memory);
  role and company claims cite ``value.job_evidence``. Every ID reported in
  ``evidence_used`` exists in one of those two input collections.
* Job-description and company-detail evidence is context only and is never
  allowed to prove a claim about the candidate.
...
from src.tools.common import pdf_page_count, run_pdflatex
from src.tools.fit_analysis.aliases import canonicalize, skill_in_text
...
def generate_cover_letter(
    ...
    *,
    ...
    with active.span(
        "cover_letter_content_generation",
        trace_metadata,
        input={
            "job_id": value.job.job_id,
    ...
    tex_path = job_dir / "cover-letter.tex"
    pdf_path = job_dir / "cover-letter.pdf"
...
def _match_required_skills(
    ...
    for skill in required_skills:
        ...
        candidates = [
            ...
            if skill_in_text(needle, item.text)
            or any(canonicalize(tag) == needle for tag in item.tags)
        ]
        if not candidates:
            continue
    ...
    for attempt in range(MAX_COMPILE_ATTEMPTS):
        ...
        if run_errors:
            active.end_span(
                ...
                output={"errors": run_errors, "pdf_created": pdf_path.is_file()},
            )
        ...
        active.end_span(
            ...
            output={"pdf_created": pdf_created, "pdf_file": pdf_path.name},
        )
