"""Evidence-grounded LaTeX resume tailoring and one-page PDF compilation."""
...
def _rewrite_experience_bullet(
    ...
    if item is not None:
        ...
        for claim in claims:
            skill = claim.claim.split(":", 1)[0].strip()
            if item.evidence_id in claim.evidence_ids or skill_in_text(
                canonicalize(skill), item.text
            ):
    ...
    already_tailored = re.match(
        ...
        r"Refocused this experience for .+? work by)\s+(.+)$",
        factual_body,
    )
    if already_tailored:
    ...
    if concise:
        label = (
            ...
            if matched_skills
            else _escape_latex(inp.job.title)
        )
        ...
        prefix = (
            f"Refocused this experience for {_escape_latex(inp.job.title)} work by "
        )
