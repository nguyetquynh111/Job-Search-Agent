...
class SkillClaim:
    ...
    sources: set[str]
    on_resume: bool
    members: list[str] = field(default_factory=list)  # tools in a skill category
    job_evidence_id: str = ""
...
# Split combined skill labels so each part can be matched.
...
def _resolve_required(
    ...
    if not chosen:
        return SkillClaim(
            skill=original, evidence_ids=[], sources=set(), on_resume=False, members=[]
        )
...
def _resolve_skill(
    ...
    # Each tuple tracks its label, resume status, sources, and evidence IDs.
    ...
    if direct_sources or direct_on_resume:
        supports.append(
            (
                ...
                direct_on_resume,
                direct_sources,
                index.ordered_ids(canonical, limit=6),
            )
        )
        ...
        if m_sources or m_on_resume:
            supports.append(
                (
                    ...
                    m_on_resume,
                    m_sources,
                    index.ordered_ids(member_canonical, limit=3),
                )
            )
    ...
    # Aligned claims cite resume evidence only.
...
def _skill_buckets(
    ...
    for original, _canonical in _dedupe_canonical(inp.job.required_skills):
        ...
        claim.job_evidence_id = job_skill_evidence_id(inp.job, original)
        if claim.on_resume:
...
def _parse_experience(text: str) -> tuple[str, str, str]:
    ...
    focus = (
        " ".join(parts[4:]).strip() if len(parts) >= 5 else " ".join(parts[1:]).strip()
    )
...
def _experience_matches(original: str, text: str) -> bool:
    ...
    return any(
        skill_in_text(canonicalize(part), text) for part in _split_parts(original)
    )
...
def _relevant_experience(
    ...
    for item in items:
        ...
        matched = [
            original
            for original in required_original
            if _experience_matches(original, item.text)
        ]
...
def _seniority(inp: AnalyzeFitInput) -> list[EvidenceClaim]:
    ...
    if years is not None and items:
    ...
    if (
        required is None
        and inp.job.years_experience_required is None
        and inp.job.experience_requirement.minimum_years is None
    ):
    ...
    job_requirement_id = (
        ...
        if (
            inp.job.experience_requirement.raw_text
            or inp.job.years_experience_required is not None
        )
        else job_evidence_id(inp.job, "description")
    )
    evidence_ids = [job_requirement_id, *[item.evidence_id for item in items[:2]]]
    return [
        EvidenceClaim(
...
def run_prepass(inp: AnalyzeFitInput, index: EvidenceIndex) -> PrePass:
    ...
    # Expand combined labels before scoring projects.
    ...
    return PrePass(
        ...
        job_text=job_text,
        job_context_evidence_id=job_evidence_id(inp.job, "description"),
        aligned=aligned,
...
def build_skill_section(
    ...
    return (
        ...
        [
            _skill_claim(entry, "evidenced_missing")
...
def build_project_section(
    ...
    if project_swap is not None and job_context_evidence_id:
        project_swap = project_swap.model_copy(
            update={
                "evidence_ids": list(
                    dict.fromkeys([job_context_evidence_id, *project_swap.evidence_ids])
                )
            }
        )
    return claims, project_swap
...
def prepass_summary(prepass: PrePass, index: EvidenceIndex) -> dict[str, object]:
    ...
    return {
        ...
        "suggested_swap": (
            {
                "remove_project": prepass.swap.swap.remove_project,
                "add_project": prepass.swap.swap.add_project,
            }
