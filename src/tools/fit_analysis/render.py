...
def build_source_labels(inp: AnalyzeFitInput) -> dict[str, str]:
    ...
    labels: dict[str, str] = {}
    for item in inp.job_evidence or build_job_evidence(inp.job):
        labels[item.evidence_id] = (
            "company details" if item.source == "company_details" else "job posting"
        )
...
def _skill_lines(
    ...
    for claim in claims:
        if inline_source:
            ...
            ids = (
                f"  _[evidence: {', '.join(claim.evidence_ids)}]_"
                if claim.evidence_ids
                else ""
            )
...
def render_fit_analysis(
    ...
    *,
    ...
    parts += _skill_lines(
        output.evidenced_missing_skills, missing_marker, labels, inline_source=True
    )
    parts.append("")
    ...
    if output.project_swap is not None:
        ...
        ids = (
            f"  _[evidence: {', '.join(swap.evidence_ids)}]_"
            if swap.evidence_ids
            else ""
        )
        parts.append(
...
def write_fit_analysis(
    ...
    *,
    ...
    md_path.write_text(
        render_fit_analysis(
            output, job, source_labels=source_labels, missing_marker=missing_marker
        ),
        encoding="utf-8",
    )
