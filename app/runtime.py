"""Streamlit runtime coordination for the Job Search Agent."""
...
SessionMapping = MutableMapping[Any, Any]


def ensure_session_defaults(session: SessionMapping) -> None:
...
def save_uploaded_inputs(
    session: SessionMapping, uploads: dict[str, Any]
) -> dict[str, str]:
...
def resume_graph_run(
    app: Any, session: SessionMapping, feedback: dict[str, Any]
) -> dict[str, Any]:
...
def store_graph_result(session: SessionMapping, result: dict[str, Any]) -> None:
    ...
    elif result.get("status") == "WAITING_FOR_REVIEW" and result.get(
        "interrupt_payload"
    ):
        session["waiting_for_review"] = True
...
def reset_demo_data(session: SessionMapping) -> None:
...
def get_graph_bundle(checkpoint_db: str) -> GraphBundle:
    ...
    from src.agent import build_agent_graph, create_sqlite_checkpointer
    from src.tracing.langfuse import TraceManager

    tracer = TraceManager()
    checkpointer, context = create_sqlite_checkpointer(checkpoint_db)
    app = build_agent_graph(checkpointer=checkpointer, tracer=tracer)
